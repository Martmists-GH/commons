...
class ConfigLoader(createMapper: () -> ObjectMapper) {
    ...
    inline fun <T> load(file: File, clazz: Class<T>, resource: InputStream) = load(file, clazz, resource.readAllBytes().decodeToString())
    fun <T> load(file: File, type: Class<T>, default: String): T {
        if (!file.exists()) {
            file.createNewFile()
